            ...
            >
                ...
                <div class="achievement-details">
                    ...
                    <div class="progress-bar-container">
                        <div class="progress-bar special-progress" :style="{ width: (achievement.currentProgress / achievement.targetProgress * 100) + '%' }"></div>
                        <span class="progress-text-overlay">{{ achievement.currentProgress }} / {{ achievement.targetProgress }}</span>
                    </div>
...
const achievementGroups = computed(() =>
    achievementStore.groups.filter(g => g.category === 'sentence')
)
...
.achievement-icon-wrapper.special-mode {
  ...
  width: 60px;
  height: 60px;
  background: #a855f7; /* Цвет сохранен */
  border-radius: 16px;
  border: 3px solid #1e1e1e;
  display: flex;

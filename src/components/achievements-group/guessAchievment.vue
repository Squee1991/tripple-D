<template>
  <div class="achievements">
    <div v-for="group in overGroups" :key="group.id" class="achievement-group-container">
          ...
          <div class="achievement-details-overall">
            ...
            <div class="progress-bar-overall-container">
              <div class="progress-bar-overall"
                   :style="{ width: (item.currentProgress / item.targetProgress * 100) + '%' }"></div>
              <span class="progress-text-overall">{{ item.currentProgress }} / {{ item.targetProgress }}</span></div>
...
const overGroups = computed(() =>
    achievementStore.groups.filter(g => g.category === 'guess')
)

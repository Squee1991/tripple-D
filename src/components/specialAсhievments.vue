...
<script setup>
    ...
    watchEffect(() => {
        const map = [
            {id: 'guessedFastWords', value: guessStore.guessedFastWords.length},
            {id: 'guessedSafeWords', value: guessStore.guessedSafeWords.length},
            {id: 'guessedOnLastTryWords', value: guessStore.guessedOnLastTryWords.length},
            {id: 'guessedPerfectWords', value: guessStore.guessedPerfectWords.length},
        ];

        map.forEach(({id, value}) => {
            const ach = allAchievements.value.find(a => a.id === id);
            if (ach) {
                ach.currentProgress = Math.min(value, ach.targetProgress);
            }
        });
    });

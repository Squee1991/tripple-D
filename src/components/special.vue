...
<script setup>
	...
	const specialTasks = ref([
		...
		},
		{
			...
			title: 'Секретное испытание',
			description: 'Выучи 1 тему всеми способами',
			progress: 5,
			max: 7,
			completed: false,
			rewarded: false,
		},
		{
			...
			title: 'Заходи каждый день в течении недели',
			description: '',
			progress: 1,
			max: 1,
			completed: true,
	...
	function rewardTask(id) {
		const task = specialTasks.value.find(t => t.id === id)
		if (task && task.completed && !task.rewarded) {
			task.rewarded = true
			alert('Ты получил особую награду!')
		}
	}
</script>

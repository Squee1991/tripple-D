  ...
  const props = defineProps({
    ...
    levelStart: {type: Number, default: null},
    levelEnd: {type: Number, default: null},
  })
  const phrases = [
  ...
  const shownLevel = ref(props.level ?? (props.levelStart ?? null))

  function animateInt(from, to, setter, duration = 900) {
    if (to === from || to === null || from === null) {
      setter(to);
      return
    }
  ...
  onMounted(() => {
    ...
    setTimeout(() => {
      ...
      if (props.levelStart !== null && props.levelEnd !== null) {
        animateInt(props.levelStart, props.levelEnd, v => (shownLevel.value = v), 700)
      } else if (props.level !== null) {
        shownLevel.value = props.level
      } else {
        shownLevel.value = null
      }
    }, 2000)
  })
  </script>

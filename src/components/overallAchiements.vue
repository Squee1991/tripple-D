...
watch(() => authStore.uid, (newValue) => {
  const ach = findAchievementById('registerAchievement')
  if (ach && newValue) {
...
watch(() => statsStore.constructedSentences, (newExp) => {
  const achievement = findAchievementById('grandmaster_sentences');
  if (achievement) {
    achievement.currentProgress = Math.min(newExp, achievement.targetProgress);
  }
}, {immediate: true});
...
onMounted(async () => {
  langStore.loadFromFirebase()
  watch(() => langStore.totalEarnedPoints, (newPoints) => {
    const ach = findAchievementById('firstArticleAward')
    if (!ach) return

    const safe = Number.isFinite(newPoints) ? newPoints : 0
    ach.currentProgress = Math.min(safe, ach.targetProgress)
  }, {immediate: true})
})
</script>

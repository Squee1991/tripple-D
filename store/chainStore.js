...
export const userChainStore = defineStore('chain', () => {
	...
	function applyLifeRegenIfNeeded() {
		...
		const now = Date.now()
		const ticks = Math.floor((now - lastLifeAtMs.value) / HOUR_MS)
		if (ticks <= 0) return
	...
	async function saveFinalProgress() {
		...
		await runTransaction(db, async (tx) => {
			...
			const answered = finished.value
				? requiredTasks.value
				: Math.min(currentIndex.value + (showResult.value ? 1 : 0), requiredTasks.value)

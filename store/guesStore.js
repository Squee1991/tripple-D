...
export const useGuessWordStore = defineStore('guessWord', () => {
    ...
    async function loadGuessProgress() {
        ...
        if (snap.exists()) {
            ...
            if (Array.isArray(data.guessedFastWords)) {
                guessedFastWords.value = data.guessedFastWords
            }
            if (Array.isArray(data.guessedSafeWords)) {
                guessedSafeWords.value = data.guessedSafeWords
            }
            if (Array.isArray(data.guessedOnLastTryWords)) {
                guessedOnLastTryWords.value = data.guessedOnLastTryWords
            }
            if (Array.isArray(data.guessedPerfectWords)) {
                guessedPerfectWords.value = data.guessedPerfectWords
            }
        }
    }
	...
	watch(win, async w => {
		if (!w || !answer.value) return
		const word = answer.value
		const alreadyGuessed = guessedWords.value.includes(word)
		if (!alreadyGuessed) {
			guessedWords.value.push(word)
			if (timeSpent.value !== null && timeSpent.value <= 20 && !guessedFastWords.value.includes(word)) {
				guessedFastWords.value.push(word)
			}
			if (attempts.value >= 10 && !guessedSafeWords.value.includes(word)) {
				guessedSafeWords.value.push(word)
			}
			if (attempts.value === 15 && !guessedPerfectWords.value.includes(word)) {
				guessedPerfectWords.value.push(word)
			}
			if (attempts.value === 1 && !guessedOnLastTryWords.value.includes(word)) {
				guessedOnLastTryWords.value.push(word)
			}
		}

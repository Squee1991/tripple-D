...
export const useQuizStore = defineStore('quiz', () => {
    ...
    const score = computed(() =>
        Object.values(userAnswers.value).filter(a => a && a.isCorrect).length
    )
    ...
    function setContext({ modeId, topicId, fileName, contentVersion, attemptId }) {
        context.value.modeId = modeId
        context.value.topicId = topicId
        context.value.fileName = fileName
        context.value.contentVersion = contentVersion
    ...
    async function saveSessionStart() {
        try {
            ...
            console.warn('saveSessionStart error:', e)
        }
    }
    ...
    async function finalizeAttempt() {
        ...
        await runTransaction(db, async (tx) => {
            ...
            const t = tSnap.exists() ? tSnap.data() : {
                attemptsCount: 0, bestScore: 0, totalAnswered: 0, totalCorrect: 0, averageAccuracy: 0,
                perfectSessionsCount: 0,
                fastestPerfectSec: null,
                hasPerfect: false,
                fastPerfectSessionsCount: 0,
            }
    ...
    async function startNewQuiz(arg) {
        let modeId, topicId, fileName, contentVersion
        if (typeof arg === 'string') {
            fileName = arg
            modeId = context.value.modeId || 'unknown'
            topicId = context.value.topicId || 'default'
            contentVersion = context.value.contentVersion || 'v1'
        } else {
            ({ modeId, topicId, fileName, contentVersion } = arg)
        }

...
const loadUserNickname = async (uid) => {
  ...
  if (snap.exists()) {
    userNicknames.value[uid] = snap.data().nickname || `Игрок...`
  }
}
...
onMounted(() => {
  ...
  unsubscribeSession = onSnapshot(sessionRef, async (docSnap) => {
    if (!docSnap.exists()) {
      ...
      setTimeout(() => router.push('/duel'), 3000);
      return
    }
...
.battle-title {
  ...
  font-size: 28px;
  color: #3e2e1f;
  text-shadow: 1px 1px 0px #fffbe8;
}

.status-message.opponent-left {
  background: #ffebe8;
  color: #a11919;
  font-weight: bold;
  padding: 8px 16px;

...
import {useSentenceDuelStore} from '../store/sentenceDuelStore.js'
import {useLocalStatGameStore} from '../store/localSentenceStore.js'

const {t} = useI18n()
const router = useRouter()
const route = useRoute()
const gameStore = useSentenceDuelStore()
...
onMounted(async () => {
    try {
        // Загружаем задачи для уровня
        await gameStore.loadLocalTasks(level)

        // Загружаем статистику
        await statStore.loadLocalStats()

        // Преобразуем предложения в задачи
        tasks.value = gameStore.localTasks
            .filter(task => task && task.original)
            .map(task => {
                const cleanSentence = task.original.toLowerCase().replace(/[.,!?;]/g, '')
                return {
        ...
        if (tasks.value.length > 0) {
            ...
            console.error("Не удалось загрузить задания. Возможно, для уровня", level, "нет предложений.")
            ...
            <div class="game-container">
                <div v-if="!finished" class="game-board">
                    ...
                    <div class="actions-container">
                        ...
                        <button v-if="!isAnswerChecked" class="btn btn-check" @click="checkAnswer">{{
                            t('prasens.check')
                            }}
                        </button>

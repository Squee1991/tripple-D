                                    ...
                                    <template v-else-if="acc.key === 'account'">
                                        <div class="subscription-status-row">
                                            ...
                                            <div class="subscription-status">
                                                <template
                                                        v-if="authStore.isPremium && !authStore.subscriptionCancelled">
                                                    <p class="active">✅ {{ t('cabinet.active') }}</p>
                                                </template>
                                                <template
                                                        v-else-if="authStore.isPremium && authStore.subscriptionCancelled">
                                        ...
                                        <template v-else-if="authStore.isPremium && authStore.subscriptionCancelled">
                                            <p class="access__text">📅 {{ t('cabinet.access') }}
                                                {{ formattedSubscriptionEndDate }}</p>
                                        </template>
                                    </template>
        ...
        <div v-if="isDeleteModalOpen" class="modal-overlay" @click.self="isDeleteModalOpen = false">
            <div class="modal-card">
                ...
                <div class="modal-actions">
                    ...
                    <button class="btn" @click="isDeleteModalOpen = false">{{
...
const handleLeveling = () => {
    ...
    if (learningStore.exp >= LEVEL_UP_XP) {
        learningStore.isLeveling++
        learningStore.exp -= LEVEL_UP_XP
    }
}
...
watch(() => learningStore.exp, (newExp, oldExp) => {
    // Вызываем функцию, когда значение опыта меняется
    handleLeveling()
}, {immediate: true}) // { immediate: true } чтобы проверить при первом запуске компонента

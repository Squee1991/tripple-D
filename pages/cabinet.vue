    ...
    <div class="cabinet-wrapper">
        <div v-if="isCancelModalOpen" class="modal-overlay" @click.self="closeCancelModal">
            <div class="modal-card">
                ...
                <p class="modal-text">Если вы отмените подписку, премиум-доступ останется до конца оплаченного
                    периода.</p>
            ...
            <section class="content-panel">
                <div class="header-surface">
                    <div class="user-block">
                        <div class="avatar-container">
                            <img v-if="authStore.avatarUrl" :src="authStore.avatarUrl" alt="Аватар"
                ...
                <div class="content-body">
                    <div v-if="activeTabKey === 'info'" class="tab-content">
                        ...
                        <div class="accordion" :class="{ open: isAccountAccordionOpen }"
                             @click="toggleAccountAccordion">
                            ...
                            <transition name="fade">
                                <div v-show="isAccountAccordionOpen" class="accordion__body" @click.stop>
                                    <template v-if="authStore.isPremium && !authStore.subscriptionCancelled">
                                        ...
                                        <button class="btn btn-danger" @click.stop="openCancelModal">Отменить подписку
                                        </button>
                                    </template>
                                    <template v-else-if="authStore.isPremium && authStore.subscriptionCancelled">
                        ...
                        <div class="accordion" :class="{ open: isSettingsAccordionOpen }"
                            ...
                            <transition name="fade">
                                <div v-show="isSettingsAccordionOpen" class="accordion__body settings__elements"
                                     @click.stop>
                                    <div v-for="settingsItem in settingsToggleItems" :key="settingsItem.key"
...
watch(() => achievementStore.lastUnlockedAward, (val) => {
    if (!val) return
    const award = awardList.value.find(a => a.title === val.title)
    if (award && award.locked) award.locked = false

    genericModalData.value = {
        title: 'Поздравляем!',
        text: `Вы получили награду «${val.title}»!`,
        img: award?.icon ?? DevelopmentIcon
    }

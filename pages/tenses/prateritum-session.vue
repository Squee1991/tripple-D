            ...
            <div v-else-if="quizStore.activeQuestion" class="quiz-content-comic">
                <div class="question-card-comic">
                    <p class="question-text-comic">
                        <span>{{ quizStore.activeQuestion.question.split('_______')[0] }}</span>
                        <span class="blank-space">{{ quizStore.selectedOption || '( ..... )' }}</span>
                        <span>{{ quizStore.activeQuestion.question.split('_______')[1] }}</span>
                    </p>
                </div>
...
<script setup>
    ...
    async function startQuiz() {
        loading.value = true;
        await quizStore.startNewQuiz('/verbs-data/praterium.json');
        loading.value = false;
    }

        ...
        try {
            ...
            if (cooldown) {
                if (nextId) {
                    ...
                    if (isNextReady) {
                        await playDialogue([
                            { speaker: 'chef', text: `Ты уже выполнил это задание. Загружаю следующее...`, status: 'success' }
                        ])
                        ...
                        await playDialogue([
                            { speaker: 'chef', text: `Следующее задание будет доступно через ${formatted}`, status: 'error' }
                        ])
                        return
                    }
                } else {
                    await playDialogue([
                        { speaker: 'chef', text: `Ты выполнил всё в этой теме!`, status: 'success' }
                    ])
                    return
                }
            }
